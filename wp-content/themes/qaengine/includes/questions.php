			...
			foreach ($this->taxonomy_args as $tax_name => $tax_args) {
				if ( isset($args['tax_input'][$tax_name]) ){
					if($tax_name == "question_category" && !term_exists( $args['tax_input'][$tax_name], 'question_category' )){
						new WP_Error(__('Category is not existed!', ET_DOMAIN));
					}
	...
	public function _convert($post, $taxonomy = true, $meta = true){
		...
		if ( $taxonomy ){
			foreach ($this->taxonomy_args as $name => $args) {
				$result[$name]	 = isset($result['ID']) ? wp_get_object_terms( $result['ID'], $name ) : '';
			}
		}
		...
		if ( $meta ){
			foreach ($this->meta_data as $key) {
				$result[$key] 	= isset($result['ID']) ? get_post_meta( $result['ID'], $key, true ) : '';
			}
		}
...
class QA_Questions extends ET_PostType {
	...
	*/
	...
	public static function convert($post){
		...
		return apply_filters('qa_question_convert', $result );
	}
	...
	public static function insert_question($title, $content, $cats, $status = "publish" , $author = 0){
		...
		if( isset($cats['question_category']) && !term_exists( $cats['question_category'], 'question_category' ) )
			 return new WP_Error(__('Category is not existed!', ET_DOMAIN));
		$data = array(
			'post_title' 		=> $title,
	...
	public static function search($data){
		...
		$data = wp_parse_args( $data, array(
			'post_type' 	=> array(self::POST_TYPE,'poll'),
	...
	*/
	...
	public static function delete($id, $force_delete = false){
		...
		if($success){
			...
			$count = et_count_user_posts($answer->post_author, 'answer');
			update_user_meta( $answer->post_author, 'et_answer_count', $count );
			//update question meta et_answers_count
			$count_answer = get_post_meta( $question->ID, 'et_answers_count', true);
			if($count_answer)
				update_post_meta( $question->ID, 'et_answers_count', $count_answer - 1);
	...
	public static function convert($post){
		...
		//Get color/title user_badge
		$user_point =   qa_get_user_point( $post->post_author );
	    ...
	    if(!empty($levels)){
	        foreach($levels as $key => $level) {
	            if( $level->qa_badge_point <= $user_point ) {
	                $user_badge_color  = $level->qa_badge_color ;
	                $user_badge_title 	= $level->post_title;
	                break;
	            }
	        }
	    }
		...
		$result->user_badge_color 	= ($user_badge_color) ? $user_badge_color : '';
		$result->user_badge_title 	= ($user_badge_title) ? $user_badge_title : '';
		...
		$result->content_filter 	= apply_filters( 'the_content', $result->post_content , $result);
		$result->content_edit       = et_the_content_edit($post->post_content);
		...
		$result->reported  			= in_array($current_user->ID,(array)get_post_meta($post->ID, 'et_users_report', true ));
		$result->check_role_best_answer = check_role_best_answer($result->parent_author, $result);
		return $result;
	}
...
/**
 * Update Hot Question when trash question
 */
function update_hot_question($post_id){
	global $wpdb;
	// get post
	$post = get_post($post_id);
	
	if($post->post_type == 'question'){
		$widget = new QA_Hot_Questions_Widget();
		$settings = $widget->get_settings();
		foreach ($settings as $key => $value) {
		    $instance_widget = $settings[$key];
		    break;
		}
		
		if(!isset($instance_widget['normal_question'])){
			// Update Transient Hot Question 			
			$hour       = 12;
			$today      = strtotime("$hour:00:00");
			$last7days  = strtotime('-7 day', $today);
			$last30days = strtotime('-30 day', $today);

			if($instance_widget['date'] == "last7days"){
				$custom = "AND post_date >= '".date("Y-m-d H:i:s", $last7days)."' AND post_date <= '".date("Y-m-d H:i:s", $today)."' ";
			} elseif ($instance_widget['date'] == "last30days") {
...
add_action('trashed_post', 'update_hot_question');

/**
 * Update et_answers_count when trash answer
 */
function qae_before_delete_answer($postid){
	global $post_type;
    if ( $post_type == 'answer' ){
    	$parent_id = wp_get_post_parent_id( $postid );
    	$count_answer = get_post_meta( $parent_id, 'et_answers_count', true);
		if($count_answer)
			update_post_meta( $parent_id, 'et_answers_count', $count_answer - 1);
    }
}
add_action( 'before_delete_post', 'qae_before_delete_answer' );