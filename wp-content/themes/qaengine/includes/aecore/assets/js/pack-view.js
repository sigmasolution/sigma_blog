...
(function(Models, Views, Collections, $, Backbone) {
	...
	Views.PackForm = Backbone.View.extend({
		tagName: 'div',
		events: {
			'change form.edit-plan input[name=sku]' : 'checkUniqueSKU',
			...
			$.ajax({
                ...
                success: function(res) {
                	if(!res.success){
                		$sku.addClass('error');
                		$sku.parent().find('label').remove();
                		$sku.parent().append('<label for="sku" class="error">'+res.msg+'</label>');
	...
	Views.Pack = Backbone.View.extend({
		...
		events: {
			'change form.add-pack-form input[name=sku]' : 'checkUniqueSKU',
