...
class QA_Front extends QA_Engine{
	...
	public function __construct(){
		...
		$this->add_filter( 'comment_post_redirect'	, 'qa_comment_redirect', 10, 2 );
	}
	

	function qa_comment_redirect( $location, $comment ) {
		global $post;
	...
	public function frontend_init($wp_rewrite){
	    ...
	    $author_slug = ae_get_option('members_slug', 'member');//apply_filters( 'qa_member_slug', 'member' ); // change slug name
	...
	public function scripts_in_footer(){
        ...
        <?php do_action('qa_before_render_script'); ?>
		...
		<script type="text/javascript" id="current_user">
			...
			?>
		 	currentUser = <?php
		 	if ($current_user->ID)
		 		echo json_encode($user_limit_info);
		 	else
	...
	public function on_add_scripts(){
		...
		$this->add_existed_script('marionette');
		$this->add_script('appengine', 			ae_get_url(). '/assets/js/appengine.js',array('jquery', 'backbone', 'underscore','plupload'));
		...
		if(et_load_mobile()){
			...
			$this->add_script('date-picker-moment', 		TEMPLATEURL . '/js/moment.min.js', array('jquery', 'underscore', 'backbone', 'site-functions'));
			$this->add_script('date-picker', 		TEMPLATEURL . '/js/bootstrap-datetimepicker.min.js', array('jquery', 'underscore', 'backbone', 'site-functions','date-picker-moment'));
			$this->add_script('color-picker', 		TEMPLATEURL . '/js/colorpicker.js', array('jquery', 'underscore', 'backbone', 'site-functions','date-picker-moment'));

			// Localize scripts
			wp_localize_script( 'site-front', 'qa_front', qa_static_texts() );

			// Add scripts for both single question and single poll
			if( is_singular( 'question' ) || is_singular( 'answer' ) || is_singular( 'poll' )){
			...
			// Add scripts for single question
			...
			// Add scripts for single poll
			if( is_singular( 'poll' )){
				$this->add_script('single-poll', 	TEMPLATEURL . '/js/single-poll.js', array('jquery', 'underscore', 'backbone', 'site-functions','site-front'));
			}
	...
	public function on_add_styles(){
		...
		if(et_load_mobile()){
			...
			$this->add_style( 'date-picker-style'	, TEMPLATEURL.'/css/bootstrap-datetimepicker.min.css' );
			$this->add_style( 'color-picker-style'	, TEMPLATEURL.'/css/colorpicker.css' );

			if(is_singular( 'question' )){
				$this->add_style('qa-shstyle', TEMPLATEURL . '/css/shCoreDefault.css');
			}
			if(is_single()){
				//enqueue style magnific
...
class QA_FrontPost extends AE_Base{
	...
	public function handle_posts(){
		...
		*
		**/
	    if( is_singular( 'question' ) || is_singular('poll') ) {
	        global $post,$user_ID;
...
class QA_Ajax extends AE_Base{
	...
	public function scroll(){
		...
		$args  = array(
            ...
            'orderby' =>  array( 'second_key' => 'DESC', 'date' => 'DESC' )
        );
		...
		if(et_load_mobile()){
			$url_question = 'mobile/template/question';
			$url_poll = 'mobile/template/poll';
		}else{
			$url_question = 'template/question';
			$url_poll = 'template/poll';
		}
		$posts = array();
		global $post;
		if($query->have_posts()){
			while($query->have_posts()){
				...
				if($post->post_type == 'question') {
                    get_template_part( $url_question, 'loop' );
                } else {
                    get_template_part( $url_poll, 'loop' );
                }
	...
	public function create(){
		try{
			...
			if( isset($args['qa_nonce']) && wp_verify_nonce( $args['qa_nonce'], 'insert_comment' )) {
				...
				$post_parent  = isset($args['post_parent']) ? QA_Questions::convert(get_post( $args['post_parent'] )) : '';
				if(!qa_user_can('add_comment') && $user_ID != $post_parent->post_author)
				...
				if(is_wp_error( $result )){
					...
					$question_link = '<a href="'. get_permalink($args['post_parent']) .'">'. get_permalink($args['post_parent']) .'</a>';
					$message = str_ireplace('[question_link]', $question_link, $message);
					$message = str_ireplace('[question_title]', $post->post_title, $message);
		        ...
		        if(ae_get_option('gg_question_captcha', false) && !current_user_can( 'administrator' ) && $_POST['method'] == 'create' && ae_get_option('gg_secret_key')){           
				...
				do_action( 'qa_before_insert_question', $args );
				...
				$result = QA_Questions::insert_question($args['post_title'], $args['post_content'], $cats, $status);
						  ...
						  QA_Questions::update_field($result, "et_pump_time", time());
						  QA_Questions::update_field($result, "et_new_post", true);
		...
		try {
			...
			switch ( $action ) {
				...
				case 'un-accept-answer':
                    ...
                    if(empty($question->et_best_answer)) {
                    ...
                    }elseif(!empty($question->et_best_answer)){
                    	if( $current_user->ID == $question->post_author || current_user_can( 'manage_options' ) || ($user_point > $approve_answer && empty($question->et_best_answer))) {
							QA_Questions::mark_answer( $_POST['post_parent'], $answerID );
				...
				case 'approve':
					...
					if( $success &&  !is_wp_error( $success ) ) {
						if($post->post_type == "question" || $post->post_type == 'poll'){
							//add points to user
	...
	public function upload_images(){
		try{
			...
			if($_FILES[$fileID]['size'] > $max_file_size*1024*1024){
				throw new Exception( sprintf(__('Image file size is too big.Size must be less than < %sMB.',ET_DOMAIN), $max_file_size) );
			}
	...
	public function delete(){
		try {
			...
			if($post->post_type == 'question') {
				...
				$redirect = get_post_type_archive_link( 'poll' );
			} else {
